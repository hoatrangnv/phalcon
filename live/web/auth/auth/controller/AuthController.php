...
use TVN\Auth\Form\AdminLoginForm;
use TVN\Auth\Form\SeekerLoginForm;
use TVN\Auth\Form\EmployerLoginForm;
use TVN\Datasource\Model\Admin;
use TVN\Datasource\Model\AuthToken;
use TVN\Datasource\Model\Seeker;
use TVN\Datasource\Model\SeekerLoginHistory;
use TVN\Datasource\Model\Employer;
use TVN\Datasource\Model\EmployerLoginHistory;
...
class AuthController extends BaseController
{
            ...
            if (!$form->isValid($this->request->getPost())) {
                ...
                switch ($channel_id) {
                    ...
                    case 3:
                        ...
                        $seeker = self::seekerLogin($email, $password);
                        if (!$seeker) {
                            $this->flashSession->error('Tài khoản đăng nhập không chính xác.');
                        } else {
                            $auth_token->status = Constant::AUTH_TOKEN_STATUS_LOGINED;
     ...
     * @author Cuong.Bui
     */
    private function seekerLogin($email, $password)
    {
        $seeker = Seeker::findFirst(array(
