...
try {
    ...
    $config = new Ini(ROOT . '/app/auth/config/parameter.ini');
    ...
    $di->setShared('db', function() use ($config) {
        ...
        if ($debug) {
            $e = new EventsManager();
            $logger = new Logger(ROOT . '/app/auth/log/db_master_' . date('Ymd') . '.log');
    ...
    $di->setShared('db_slave', function() use ($config) {
        ...
        if ($debug) {
            $e = new EventsManager();
            $logger = new Logger(ROOT . '/app/auth/log/db_slave_' . date('Ymd') . '.log');
    ...
    $di->setShared('router', function() {
        ...
        $routers = new Ini(ROOT . '/app/auth/config/router.ini');
    ...
    $di->setShared('view', function() {
        $view = new View();
        $view->setViewsDir(ROOT . '/app/auth/view/');
        return $view;
    });
    ...
    $di->setShared('modelsMetadata', function() use ($config) {
        $metaData = new MetaDataApc(array(
            ...
            'lifetime' => $config->cache->metadata_lifetime
        ));
        return $metaData;
    });
    ...
    $di->setShared('dispatcher', function() {
        ...
        $dispatcher->setDefaultNamespace('ITECH\Auth\Controller\\');
    ...
    $di->setShared('logger', function() {
        $logger = new Logger(ROOT . '/app/auth/log/debug_' . date('Ymd') . '.log');
        return $logger;
    });
    ...
    $application->registerModules(array(
        'auth' => array(
            'className' => 'ITECH\Auth\Module',
            'path' => ROOT . '/app/auth/Module.php'
        ),
        'datasource' => array(
